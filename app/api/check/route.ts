...
export async function GET(request: Request) {
  ...
  // Database Connection: Attempt to load the Background Scraper Matrix from Redis
  let PRICING_MATRIX = FALLBACK_MATRIX;
  if (redis) {
    try {
      const cached = await redis.get('domain_pricing_matrix');
      if (cached) {
        PRICING_MATRIX = JSON.parse(cached as string);
      }
    } catch (e) {
      console.error("Redis fetch failed, using fallback:", e);
    }
  }
